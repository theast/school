...
%%%% Connect to a node
%%%%%%%%%%%%%%%%%%%%%%%
loop(St = #cl_st{nick = Nick}, {connect, Server}) ->
  case catch(genserver:request(list_to_atom(Server), {connect, Nick, self(), St})) of
    {'EXIT', _} ->
      {{error, server_not_reached, atom_to_list(server_not_reached)}, St};
    
    ok ->
      {ok, St#cl_st{server = list_to_atom(Server)}};
...
loop(St = #cl_st{nick = Nick, server = Server, numberOfChannels = NoChannels}, {join, Channel}) ->
  case catch(genserver:request(Server, {join, Nick, list_to_atom(Channel)})) of
    {'EXIT', _} ->
      {{error, server_not_reached, atom_to_list(server_not_reached)}, St};
...
loop(St = #cl_st{nick = Nick, numberOfChannels = NoChannels}, {leave, Channel}) ->
  case catch(genserver:request(list_to_atom(Channel), {leave, Nick})) of
    {'EXIT', _} ->
      {{error, server_not_reached, atom_to_list(server_not_reached)}, St};
...
loop(St = #cl_st{nick = Nick}, {msg_from_GUI, Channel, Msg}) ->
  case catch(genserver:request(list_to_atom(Channel), {msg_from_GUI, Nick, Msg})) of
    {'EXIT', _} ->
      {{error, server_not_reached, atom_to_list(server_not_reached)}, St};
