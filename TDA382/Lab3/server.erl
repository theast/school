-module(server).
-export([initial_channel_state/2, loop/2, initial_state/1, sendmsg/4]).
...
loop(St = #server_st{users = Users}, {connect, Nick, Pid, State}) ->
  case lists:member(Nick, Users) of
    true ->  
      {{error, user_already_connected}, St};

    false -> 
      UserPid = spawn(fun () -> client:user(Pid, State) end),
...
loop(St = #server_st{channels = Channels}, {join, Nick, Channel}) ->
  case lists:member(Channel, Channels) of 
    true -> 
      case catch(genserver:request(Channel, {join_channel, Nick})) of
        ok ->   
          {ok, St};
        {error, Reason} ->
          {{error, Reason}, St}
...
loop(St = #channel_st{users = Users, channelName = ChannelName}, {msg_from_GUI, Nick, Msg}) ->
  case lists:member(Nick, Users) of
    true ->
        OtherUsers = lists:delete(Nick, Users),
        spawn(fun() -> lists:foreach(fun(P) -> spawn(fun () -> sendmsg(P, ChannelName, Nick, Msg) end) end, OtherUsers) end),
  ...
  sendmsg (UserPid, ChannelName, Nick, Msg) ->
  spawn(fun() -> UserPid ! {msg, {ChannelName, Nick, Msg}} end).
